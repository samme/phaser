...
const CSSRenderer = new Class({
    ...
    {
        const { renderNode, tintNode } = sprite;
        ...
        const {displayOriginX} = sprite;
        const {displayOriginY} = sprite;
        ...
        {
            // TODO
            // sprite.mask.preRenderCSS(this, sprite, camera);
        }

        if (frameWidth > 0 && frameHeight > 0)
        {
            renderNode.setAlpha(sprite.alpha);
            renderNode.setBlendMode(sprite.blendMode);
            ...
            {
                renderNode.setXY(sprite.x + x, sprite.y + y);
            }
            else
            {
                renderNode.setTSR(sprite.x + x, sprite.y + y, sprite.scaleX, sprite.scaleY, sprite.rotation);
            ...
            else if (sprite.filters._filters.length > 0)
            {
                renderNode.setProperty('filter', sprite.filters.getCSS());
            }
            else
            {
        ...
        {
            // TODO
            // sprite.mask.postRenderCSS(this, sprite, camera);
        }
